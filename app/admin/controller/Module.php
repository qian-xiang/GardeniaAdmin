...
class Module extends AdminController
{
    ...
    public function install() {
        ...
        $runTimePath = runtime_path().'module'.DIRECTORY_SEPARATOR.time();
        ...
        while ($fileEntry = zip_read($zipHandle)) {
            ...
            if (!$count) {
                ...
                error_json('模块文件不合法，请勿上传无关的文件或目录');
            }
            zip_entry_close($fileEntry);
        }
        zip_close($zipHandle);
        ...
        if ($intersect !== $mustKeys) {
            error_json('模块文件不合法，'.implode(',',$mustKeys).'必传');
        }
        // 检测结束
        // 开始将压缩文件中写入到磁盘中
        // 该过程留待日后看看是否可以进一步优化
        ...
        rename($runTimePath.DIRECTORY_SEPARATOR.'info.ini',base_path().$moduleName.DIRECTORY_SEPARATOR.'info.ini');

        $publicPath = $runTimePath.DIRECTORY_SEPARATOR.'assets';
        if (file_exists($publicPath)) {
            $newPath = public_path().DIRECTORY_SEPARATOR.'static'.DIRECTORY_SEPARATOR.'core'.DIRECTORY_SEPARATOR.'module'.DIRECTORY_SEPARATOR.$moduleName.DIRECTORY_SEPARATOR;
        ...
        // 将插件的菜单url加入到菜单规则表中
//        $addonUrl = 'module/'.$moduleName.'/admin/index';
//        Db::startTrans();
